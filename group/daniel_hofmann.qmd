...
description: "Former master student in Tristan Bereau's group at the Institute for Theoretical Physics, Heidelberg University."
format:
  html:
    echo: false
    keep-hidden: true
categories:
  - master student
  - alumnus
image: ../static/daniel_hofmann.jpg
about:
  id: hero-heading
  template: solana
  image: ../static/daniel_hofmann.jpg
...
my_papers = papers.filter(function(p) {
    return p.authors.map(a => a.full_name).includes("Daniel Hofmann")
})
```
...
collaborators = Array.from(
    new Set(
        ...
        .filter(function(a) {
            return a.full_name != "Daniel Hofmann"
            }
        )
    )
)
